...
A Data Provider is an organisation or individual that has a Telematics Account which can be connected to a Tenancy to allow data for Devices intalled in vehicle's or other physical assets to be accessed by a Data Consumer.
...
A Tenancy is a PathStack account created by a Data Consumer that will have one or more Connections established by Data Providers.
...
A User with API or Websocket access to a Tenancy may access data for any Connection within the tenancy, subject to the Data Sharing Rules established by each Data Provider.
